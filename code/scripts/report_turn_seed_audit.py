...
from postflop_trees import build_flop_tree_spec_library, build_turn_tree_spec_library

PROJECT_ROOT = Path(__file__).resolve().parents[2]
RAW_ROOT = PROJECT_ROOT / "data" / "hand_histories" / "raw"
AUDIT_ROOT = PROJECT_ROOT / "data" / "hand_histories" / "turn_seed_audit"
...
def build_audit(hero_name: str = DEFAULT_HERO_NAME, max_examples: int = 3, limit_files: int | None = None) -> dict[str, Any]:
    ...
    seed_family_map = turn_library["seed_family_map"]

    family_bucket_summary: dict[str, dict[str, dict[str, Any]]] = defaultdict(
        lambda: defaultdict(
            lambda: {
                "hand_count": 0,
                "active_mapped_seed_counts": defaultdict(int),
                "active_unmapped_seed_counts": defaultdict(int),
                "removed_mapped_seed_counts": defaultdict(int),
                "reachable_turn_family_counts": defaultdict(int),
            }
        )
    )
    turn_family_source_counts: dict[str, dict[str, int]] = defaultdict(lambda: defaultdict(int))
    unmapped_turn_seed_family_counts: dict[str, dict[str, int]] = defaultdict(lambda: defaultdict(int))
    ...
    hands_with_active_unmapped_turn_seed = 0
    ...
    for path in raw_files:
        ...
        for hand in split_hands(read_file(path)):
            ...
            if family_id not in seed_family_map:
                continue

            hands_with_turn_seed_map += 1
            board_bucket = tag.get("board_bucket") or "UNSPECIFIED"
            mapped_seed_targets = seed_family_map[family_id]
            ...
            active_mapped_seed_ids = [seed_id for seed_id in active_turn_seed_ids if seed_id in mapped_seed_targets]
            active_unmapped_seed_ids = [seed_id for seed_id in active_turn_seed_ids if seed_id not in mapped_seed_targets]
            reachable_turn_families = sorted({mapped_seed_targets[seed_id] for seed_id in active_mapped_seed_ids})

            if active_mapped_seed_ids:
                hands_with_active_mapped_turn_seed += 1
            else:
                hands_pruned_to_no_active_mapped_turn_seed += 1
            if active_unmapped_seed_ids:
                hands_with_active_unmapped_turn_seed += 1

            bucket_summary = family_bucket_summary[family_id][board_bucket]
            bucket_summary["hand_count"] += 1
            for seed_id in active_mapped_seed_ids:
                bucket_summary["active_mapped_seed_counts"][seed_id] += 1
            for seed_id in active_unmapped_seed_ids:
                bucket_summary["active_unmapped_seed_counts"][seed_id] += 1
                unmapped_turn_seed_family_counts[seed_id][family_id] += 1
            for seed_id in sorted(removed_turn_seed_ids):
                if seed_id in mapped_seed_targets:
                    bucket_summary["removed_mapped_seed_counts"][seed_id] += 1
            for turn_family_id in reachable_turn_families:
                bucket_summary["reachable_turn_family_counts"][turn_family_id] += 1
                turn_family_source_counts[turn_family_id][family_id] += 1
                ...
                bucket_examples.append(
                    {
                        ...
                        "active_mapped_turn_seeds": active_mapped_seed_ids,
                        "removed_mapped_turn_seeds": sorted(seed_id for seed_id in removed_turn_seed_ids if seed_id in mapped_seed_targets),
                        "active_unmapped_turn_seeds": active_unmapped_seed_ids,
    ...
    return {
        "meta": {
            ...
            "hands_with_active_unmapped_turn_seed": hands_with_active_unmapped_turn_seed,
        ...
        "turn_family_source_counts": {
            turn_family_id: dict(sorted(source_counts.items()))
            for turn_family_id, source_counts in sorted(turn_family_source_counts.items())
        },
        "unmapped_turn_seed_family_counts": {
            seed_id: dict(sorted(source_counts.items()))
            for seed_id, source_counts in sorted(unmapped_turn_seed_family_counts.items())
        },
        "family_bucket_summary": {
            family_id: {
                bucket: {
                    "hand_count": summary["hand_count"],
                    "active_mapped_seed_counts": dict(sorted(summary["active_mapped_seed_counts"].items())),
                    "active_unmapped_seed_counts": dict(sorted(summary["active_unmapped_seed_counts"].items())),
                    "removed_mapped_seed_counts": dict(sorted(summary["removed_mapped_seed_counts"].items())),
...
def render_audit_text(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Hands with active unmapped turn seed: {meta['hands_with_active_unmapped_turn_seed']}",
    ...
    for turn_family_id, source_counts in report["turn_family_source_counts"].items():
        total = sum(source_counts.values())
        lines.append(f"- {turn_family_id}: {total}")
        for source_family_id, count in source_counts.items():
            lines.append(f"  - {source_family_id}: {count}")

    lines.extend(["", "Unmapped active turn seeds", "--------------------------"])
    for seed_id, source_counts in report["unmapped_turn_seed_family_counts"].items():
        total = sum(source_counts.values())
        lines.append(f"- {seed_id}: {total}")
        for source_family_id, count in source_counts.items():
            lines.append(f"  - {source_family_id}: {count}")

    lines.extend(["", "Source family / board bucket detail", "---------------------------------"])
    for family_id, bucket_summaries in report["family_bucket_summary"].items():
        ...
        for bucket, summary in bucket_summaries.items():
            reachable = ", ".join(f"{turn_family}={count}" for turn_family, count in summary["reachable_turn_family_counts"].items()) or "none"
            active = ", ".join(f"{seed_id}={count}" for seed_id, count in summary["active_mapped_seed_counts"].items()) or "none"
            unmapped = ", ".join(f"{seed_id}={count}" for seed_id, count in summary["active_unmapped_seed_counts"].items()) or "none"
            removed = ", ".join(f"{seed_id}={count}" for seed_id, count in summary["removed_mapped_seed_counts"].items()) or "none"
            lines.append(f"  - {bucket}: hands={summary['hand_count']} | turn_families={reachable} | active={active} | unmapped={unmapped} | removed={removed}")
    ...
    for family_id, bucket_rows in report["examples"].items():
        ...
        for bucket, rows in bucket_rows.items():
            ...
            for row in rows:
                ...
                active = ", ".join(row.get("active_mapped_turn_seeds") or []) or "none"
                unmapped = ", ".join(row.get("active_unmapped_turn_seeds") or []) or "none"
                removed = ", ".join(row.get("removed_mapped_turn_seeds") or []) or "none"
                targets = ", ".join(row.get("reachable_turn_families") or []) or "none"
                lines.append(f"    - {flop} | {row['matchup_id']} | active={active} | unmapped={unmapped} | removed={removed} | targets={targets} | {row['file']}")

    return "\n".join(lines)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit real tagged flop hands by reachable turn seeds and turn families.")
...
def main() -> None:
    ...
    print(f"Hands with active mapped turn seed: {report['meta']['hands_with_active_mapped_turn_seed']}")
